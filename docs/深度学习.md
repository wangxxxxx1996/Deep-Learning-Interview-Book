...
如果输入空间和输出空间的方差差别较大，即数据空间分布差异较大，那么在反向传播时可能会引起梯度消失或爆炸问题。比如，当输入空间稀疏，输出空间稠密时，将在输出空间计算得到的误差反向传播给输入空间时，这个误差可能会显得微不足道，从而引起梯度消失。而当输入空间稠密，输出空间稀疏时，将误差反向传播给输入空间，就可能会引起梯度爆炸，使得模型震荡。
...
将上式代入$\sigma^{l}$和$\sigma^{l+1}$的关系式：
...
在机器学习中，无论是分类还是回归，都可能存在由于特征过多而导致的过拟合问题。解决的办法有：
...
<img width="572" alt="image" src="https://user-images.githubusercontent.com/69800141/178901849-1eaf263a-187c-422e-b21f-8411772b0f08.png">

2. L2正则化

L1参数正则化是通过向目标函数添加正则项$\Omega (\theta)={\lVert w \rVert}_2^2$，使权重更加靠近原点。

![image](https://user-images.githubusercontent.com/69800141/178971903-21331664-beaf-4c84-969e-942590b29abe.png)

对其求偏导：

![image](https://user-images.githubusercontent.com/69800141/178972057-fc973f17-5800-4a81-969a-5052b7a7cb77.png)

然后得到梯度下降的表达式如下：

![image](https://user-images.githubusercontent.com/69800141/178972169-691f96a8-9f67-49c7-b8b7-0013302330ba.png)

![image](https://user-images.githubusercontent.com/69800141/178972330-c722176d-a7e1-4548-b6d1-a36e76a6ac8f.png)

也就是说权值进行了衰减。

**为什么权值衰减就能防止overfitting呢？**

（1）过拟合的时候系数会很大。

过拟合就是拟合函数需要顾及每一个点，最终形成的拟合函数波动很大。在某些很小的区间里，函数值的变化很剧烈。这就意味着在某些小区间里的导数值非常大，由于自变量值可大可小，所以只有系数足够大，才能保证导数值很大。

<img width="239" alt="image" src="https://user-images.githubusercontent.com/69800141/178973366-a353fdc2-019a-480d-9408-a01883c39e4d.png">
